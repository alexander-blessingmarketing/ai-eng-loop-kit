...
- **You have no browser.** Verify over HTTP against the running app (`curl`), through the test suites, and in the source. Cross-browser, responsive rendering, and anything needing DevTools cannot be checked here — mark them `[!] NOT VERIFIED` with the reason; `/e2e-tests` covers them.
