...
Read `mode` from `.ai-eng-kit` first. When it is `existing`, `docs/data-model.md` and `docs/app-shell.md` are very likely still the empty templates — **`/init` leaves them empty on purpose** rather than filling them from a skim of the repo. That means *not mapped yet*, **never** *nothing exists*, and the difference decides whether this design is usable:
...
- **Look before you design.** Read the schema where this project keeps it — migrations, models, an ORM directory, the database itself if `probe` says how to reach it. Base the design on what you actually read.
...
- In this stack: `supabase` (Auth, SSR, RLS), `supabase-postgres-best-practices` (schema/query design), `stripe-best-practices` (payments). Load the matching skill before you settle the approach for that part — memorized API knowledge goes stale.
...
**What the stack already gives you (say so, don't rebuild it):** Supabase Auth rate-limits its own endpoints. Token requests (`/auth/v1/token`, i.e. password sign-in) and verification requests are limited **per IP address and are not customizable**; the email-, OTP-, signup-confirmation- and password-reset-related limits **are** customizable — hosted under **Authentication → Rate Limits** in the dashboard, locally in `supabase/config.toml` under `[auth.rate_limit]`.

**What it does not cover** — and therefore what your design has to decide:
...
- **Anything you wrote yourself.** A custom login route, a server action checking an invite code or a password-protected share link gets no Supabase limit at all — it needs an app-level throttle (see `docs/production/rate-limiting.md`, Upstash-based, keyed per IP **and** per account).

**Decide and record these, with values:**
- **CAPTCHA** on sign-in / sign-up / password-reset forms — Supabase supports hCaptcha and Cloudflare Turnstile (dashboard: Authentication → Attack Protection; locally `[auth.captcha]`). This is the strongest available lever against automated guessing on the built-in endpoints. Recommend it for any public signup.
...
- **Password policy** — minimum length (≥ 8, longer is better), required character classes, and **leaked-password protection** (Supabase checks against HaveIBeenPwned; note it is a paid-plan feature so the user can decide).
...
- [ ] If the feature checks a credential: brute-force protection designed with concrete values (throttle limits + where enforced, CAPTCHA decision, password policy, no account enumeration) — Supabase's built-in per-IP limits named as what they do and don't cover; any custom auth route given its own throttle
...
- [ ] For third-party integrations (Supabase, Stripe, …): the matching domain skill was consulted and its best practices shaped the design decisions
- [ ] Dependencies listed
