...
- Standing up the local Supabase stack (`supabase init`/`start`), wiring clients, `.env.local`, the migrations workflow → that's **`/verify-setup`'s** job. If the user asks to spec "infrastructure setup", redirect: "That's setup, not a feature — `/verify-setup` handles it. Let's spec the first capability your users actually get."
...
- **Accounts/auth IS a real feature** — and it *includes its data foundation*. The "User Accounts & Auth" spec covers signup/login **plus** the `profiles` table, the per-user RLS pattern, and the signup→profile trigger, framed as user value ("As a user I want only I can see my data"). Don't split that data foundation into a separate backend-only spec.
